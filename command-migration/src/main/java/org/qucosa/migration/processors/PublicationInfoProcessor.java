...
public class PublicationInfoProcessor extends MappingProcessor {
    ...
    private final PublicationInfoMapping pm = new PublicationInfoMapping();

    @Override
    public void process(Document opus, ModsDefinition mods, InfoType info) throws Exception {
        if (pm.mapLanguageElement(opus, mods)) signalChanges(MODS_CHANGES);
        if (pm.mapOriginInfoElements(opus, mods)) signalChanges(MODS_CHANGES);
    }
